...
from test_d3qn import D3QN
from StarCraft2Env import StarCraft2Env

from Zerg_bot import worker
...
def main():
    ...
    agent = D3QN(alpha=0.0003, state_dim=128, information_dim=27, action_dim=env.action_space.n,
                fc2_dim=256, ckpt_dir=args.ckpt_dir, gamma=0.99, tau=0.005, epsilon=1.0,
                 eps_end=0.05, eps_dec=5e-4, max_size=1000, batch_size=1)
    create_directory(args.ckpt_dir, sub_dirs=['test_Q_eval', 'test_Q_target'])
    ...
    for episode in range(args.max_episodes):
        ...
        while not done:
            ...
            # print(information)
            action = agent.choose_action(observation, information, isTrain=True)
            # env.step(action)
            lock.acquire()
            ...
            if done:
                if res.name == 'Victory':
                    reward += 50
            agent.remember(observation, information, action, reward, observation_, information_, done)
            agent.learn()
            total_reward += reward
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt_dir', type=str, default='checkpoints/testD3QN/')
