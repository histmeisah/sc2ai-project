...
# from utils import information_process,learn_information_process
...
class DuelingDeepQNetwork(nn.Module):
    def __init__(self, alpha, state_dim, information_dim, action_dim, fc2_dim):
        super(DuelingDeepQNetwork, self).__init__()
        self.resnet = ResNet18()
        self.transformer = Transformer(input_dim=information_dim,output_dim=128)
    ...
    def forward(self, state, information):
        ...
        x = torch.relu(self.resnet(state))
...
class D3QN:
    def __init__(self, alpha, state_dim, information_dim, action_dim, fc2_dim, ckpt_dir,
    ...
    def save_models(self, episode):
        self.q_eval.save_checkpoint(self.checkpoint_dir + 'test_Q_eval/D3QN_q_eval_{}.pth'.format(episode))
        print('Saving Q_eval network successfully!')
        self.q_target.save_checkpoint(self.checkpoint_dir + 'test_Q_target/D3QN_Q_target_{}.pth'.format(episode))
        print('Saving Q_target network successfully!')

    def load_models(self, episode):
        self.q_eval.load_checkpoint(self.checkpoint_dir + 'test_Q_eval/D3QN_q_eval_{}.pth'.format(episode))
        print('Loading Q_eval network successfully!')
        self.q_target.load_checkpoint(self.checkpoint_dir + 'test_Q_target/D3QN_Q_target_{}.pth'.format(episode))
