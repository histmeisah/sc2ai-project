...
class WorkerRunshBot(BotAI):
    ...
    def get_information(self):
        ...
        information = {'work_supply': self.worker_supply,  # 农民数量
                       ...
                       'zealot_count': self.units(UnitTypeId.ZEALOT).amount + self.units(
                           UnitTypeId.CHANGELINGZEALOT).amount,
                       ...
                       'protoss_ground_armor_1_research': self.already_pending_upgrade(
                           UpgradeId.PROTOSSGROUNDARMORSLEVEL1),
                       'protoss_ground_armor_2_research': self.already_pending_upgrade(
                           UpgradeId.PROTOSSGROUNDARMORSLEVEL2),
                       'protoss_ground_armor_3_research': self.already_pending_upgrade(
                           UpgradeId.PROTOSSGROUNDARMORSLEVEL3),
                       'protoss_ground_weapon_1_research': self.already_pending_upgrade(
                           UpgradeId.PROTOSSGROUNDWEAPONSLEVEL1),
                       'protoss_ground_weapon_2_research': self.already_pending_upgrade(
                           UpgradeId.PROTOSSGROUNDWEAPONSLEVEL2),
                       'protoss_ground_weapon_3_research': self.already_pending_upgrade(
                       ...
                       'tempestground_attack_research': self.already_pending_upgrade(
                       ...
                       'OBSERVERGRAVITICBOOSTER_research': self.already_pending_upgrade(
                       ...
                       'planning_protoss_air_weapon_1_research': self.already_pending(
                           UpgradeId.PROTOSSAIRWEAPONSLEVEL1),
                       'planning_protoss_air_weapon_2_research': self.already_pending(
                           UpgradeId.PROTOSSAIRWEAPONSLEVEL2),
                       'planning_protoss_air_weapon_3_research': self.already_pending(
                           UpgradeId.PROTOSSAIRWEAPONSLEVEL3),
                       'planning_protoss_ground_armor_1_research': self.already_pending(
                           UpgradeId.PROTOSSGROUNDARMORSLEVEL1),
                       'planning_protoss_ground_armor_2_research': self.already_pending(
                           UpgradeId.PROTOSSGROUNDARMORSLEVEL2),
                       'planning_protoss_ground_armor_3_research': self.already_pending(
                           UpgradeId.PROTOSSGROUNDARMORSLEVEL3),
                       'planning_protoss_ground_weapon_1_research': self.already_pending(
                           UpgradeId.PROTOSSGROUNDWEAPONSLEVEL1),
                       'planning_protoss_ground_weapon_2_research': self.already_pending(
                           UpgradeId.PROTOSSGROUNDWEAPONSLEVEL2),
                       'planning_protoss_ground_weapon_3_research': self.already_pending(
                           UpgradeId.PROTOSSGROUNDWEAPONSLEVEL3),
                       ...
                       'planning_tempestground_attack_research': self.already_pending(
                           UpgradeId.TEMPESTGROUNDATTACKUPGRADE),
                       ...
                       'planning_OBSERVERGRAVITICBOOSTER_research': self.already_pending(
                           UpgradeId.OBSERVERGRAVITICBOOSTER),
        ...
        information = information_process(information)
        return information
    ...
    async def on_step(self, iteration: int):
        ...
        for structure in self.structures:
            ...
            if structure.type_id == UnitTypeId.COMMANDCENTER or UnitTypeId.NEXUS or UnitTypeId.HATCHERY or UnitTypeId.LAIR or UnitTypeId.HIVE:
                c = [255, 255, 175]
            else:
                c = [0, 255, 175]
        ...
        for unit in self.units:
            pos = unit.position
            if unit.type_id == UnitTypeId.DARKTEMPLAR:
                c = [255, 0, 0]
            else:
                c = [175, 255, 0]
        ...
        cv2.waitKey(1)
        self.lock.acquire()
        information_cpu = information.cpu()
        self.transaction.update({'information': information_cpu})
        ...
        if action == 0:
            ...
            if self.structures(UnitTypeId.PYLON).exists and self.units(
                    UnitTypeId.PROBE).exists and self.structures(
                UnitTypeId.NEXUS).exists and self.structures(UnitTypeId.ROBOTICSFACILITY).exists and self.structures(
                UnitTypeId.ROBOTICSBAY).exists:
            ...
            print(f'action={action}')
            if self.structures(UnitTypeId.PYLON).exists and self.units(UnitTypeId.PROBE).exists and self.structures(
                    UnitTypeId.NEXUS).exists and self.structures(UnitTypeId.TWILIGHTCOUNCIL).exists and self.structures(
                UnitTypeId.TEMPLARARCHIVE).exists:
                vt = self.structures(UnitTypeId.TEMPLARARCHIVE).random
                abilities = await self.get_available_abilities(vt)
                if self.structures(UnitTypeId.TEMPLARARCHIVE).ready and self.already_pending(
                        UpgradeId.PSISTORMTECH) == 0:
                    if self.can_afford(
...
def worker(transaction, lock):
    ...
    res = run_game(maps.get(laddermap_2023[0]),
                   [Bot(Race.Protoss, WorkerRunshBot(transaction, lock)), Computer(Race.Terran, Difficulty.Medium)],
                   realtime=False)
